...
class CcdataOnChainDexApiClient(CcdataBaseApiClient):
    """
    A client for interacting with the CryptoCompare (ccdata) "On-Chain DEX API".
    """

    def __init__(self, api_key: str = None, base_url: str = None):
        """
        Initializes the On-Chain DEX API client.
        ...
        if not self.api_key:
            logger.warning(
                "CCDATA_API_KEY is not set. Some On-Chain DEX API calls may fail."
            )


if __name__ == "__main__":
    print("Attempting to initialize CcdataOnChainDexApiClient...")
    ...
    try:
        client = CcdataOnChainDexApiClient()
        print("CcdataOnChainDexApiClient initialized successfully.")
