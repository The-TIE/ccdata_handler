...
class CcdataOnChainCoreApiClient(CcdataBaseApiClient):
    """
    A client for interacting with the CryptoCompare (ccdata) "On-Chain Core API".
    """

    def __init__(self, api_key: str = None, base_url: str = None):
        """
        Initializes the On-Chain Core API client.
        ...
        if not self.api_key:
            logger.warning(
                "CCDATA_API_KEY is not set. Some On-Chain Core API calls may fail."
            )


if __name__ == "__main__":
    print("Attempting to initialize CcdataOnChainCoreApiClient...")
    ...
    try:
        client = CcdataOnChainCoreApiClient()
        print("CcdataOnChainCoreApiClient initialized successfully.")
