...
class MinApiTradingSignalsApiClient(CcdataBaseApiClient):
    """
    A client for interacting with the CryptoCompare (ccdata) 'Trading Signals' endpoints on min-api.
    """

    def __init__(self, api_key: str = None, base_url: str = None):
        """
        Initializes the Trading Signals API client.
        ...
        if not self.api_key:
            logger.warning(
                "CCDATA_API_KEY is not set. Some Trading Signals API calls may fail."
            )
