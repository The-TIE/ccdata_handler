...
class MinApiHistoricalApiClient(CcdataBaseApiClient):
    """
    A client for interacting with the CryptoCompare (ccdata) 'Historical' endpoints on min-api.
    """

    def __init__(self, api_key: str = None, base_url: str = None):
        """
        Initializes the Historical API client.
        ...
        if not self.api_key:
            logger.warning(
                "CCDATA_API_KEY is not set. Some Historical API calls may fail."
            )
