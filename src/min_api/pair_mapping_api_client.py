...
class MinApiPairMappingApiClient(CcdataBaseApiClient):
    """
    A client for interacting with the CryptoCompare (ccdata) 'Pair Mapping' endpoints on min-api.
    """

    def __init__(self, api_key: str = None, base_url: str = None):
        """
        Initializes the Pair Mapping API client.
        ...
        if not self.api_key:
            logger.warning(
                "CCDATA_API_KEY is not set. Some Pair Mapping API calls may fail."
            )
