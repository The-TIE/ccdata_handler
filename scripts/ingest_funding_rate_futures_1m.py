...
def get_last_ingested_datetime(
    ...
    query = f"""
        SELECT MAX(datetime)
        FROM market.cc_futures_funding_rate_ohlc_1m
...
def ingest_minute_funding_rate_data_for_instrument(
    ...
    """
    Fetches minute funding rate data for a specific futures instrument and ingests it into the database.
    ...
    """
    table_name = "market.cc_futures_funding_rate_ohlc_1m"
    
    # API limits
    max_limit_per_call = 2000 # Max limit for minute funding rate data (approx 33 hours)
    ...
    if last_datetime_in_db:
        ...
        if first_funding_rate_update_datetime:
            ...
            # Fallback to max_limit_per_call minutes ago if first_funding_rate_update_datetime is not available
    ...
    # This is the minimum of one minute ago and the instrument's last funding rate update datetime
    effective_to_ts_datetime = one_minute_ago_utc
    if last_funding_rate_update_datetime and last_funding_rate_update_datetime < one_minute_ago_utc:
        effective_to_ts_datetime = last_funding_rate_update_datetime
        ...
        try:
            ...
            if data and data.get("Data"):
                ...
                if records:
                    ...
                    logger.info(
                        f"Successfully ingested {len(records)} minute funding rate records for {mapped_instrument} on {market}."
                    )
                    ...
                    logger.info(
                        f"No new minute funding rate data to ingest for {mapped_instrument} on {market} in this batch."
                    )
            else:
                logger.warning(f"No data received for {mapped_instrument} on {market} for this batch.")
        except Exception as e:
            logger.error(
                f"Error ingesting minute funding rate data for {mapped_instrument} on {market}: {e}"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Ingest minute funding rate futures data for specified or all exchanges and instruments."
    )
    parser.add_argument(
        "--exchanges",
        type=str,
    ...
    print(
        "Attempting to ingest minute funding rate futures data..."
    )
    record_rate_limit_status("ingest_funding_rate_futures_1m", "pre")
    ...
    for market, mapped_instrument, last_funding_rate_update_datetime, first_funding_rate_update_datetime in instruments_to_process:
        ingest_minute_funding_rate_data_for_instrument(
    ...
    logger.info("Minute funding rate futures data ingestion completed.")
    record_rate_limit_status("ingest_funding_rate_futures_1m", "post")
