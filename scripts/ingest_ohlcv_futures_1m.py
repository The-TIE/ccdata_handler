...
def get_futures_instruments_from_db(
    ...
    Returns a list of (exchange_internal_name, mapped_instrument_symbol, last_trade_datetime, first_trade_datetime) tuples.
    """
    if not exchanges:
...
def ingest_minute_ohlcv_data_for_instrument(
    ...
    """
    table_name = "market.cc_futures_ohlcv_1m"
    
    # API limits
    max_limit_per_call = 2000 # Max limit for minute OHLCV data (approx 33 hours)
    ...
    if last_datetime_in_db:
        ...
        if first_trade_datetime:
            start_datetime_to_fetch = first_trade_datetime
            logger.info(
                f"No existing data for {mapped_instrument} on {market}. Backfilling from first trade datetime: {start_datetime_to_fetch.strftime('%Y-%m-%d %H:%M:%S UTC')}."
            )
        else:
            # Fallback to max_limit_per_call minutes ago if first_trade_datetime is not available
            ...
            logger.info(
                f"No existing data for {mapped_instrument} on {market} and no first trade datetime. Backfilling from {start_datetime_to_fetch.strftime('%Y-%m-%d %H:%M:%S UTC')}."
            )
        ...
        try:
            ...
            if data and data.get("Data"):
                ...
                if records:
                    ...
                    logger.info(
                        f"Successfully ingested {len(records)} minute OHLCV records for {mapped_instrument} on {market}."
                    )
                    ...
                    logger.info(
                        f"No new minute OHLCV data to ingest for {mapped_instrument} on {market} in this batch."
                    )
            else:
                logger.warning(f"No data received for {mapped_instrument} on {market} for this batch.")
        except Exception as e:
            logger.error(
                f"Error ingesting minute OHLCV data for {mapped_instrument} on {market}: {e}"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Ingest minute OHLCV futures data for specified or all exchanges and instruments."
    )
    parser.add_argument(
        "--exchanges",
        type=str,
    ...
    print(
        "Attempting to ingest minute OHLCV futures data..."
    )
    record_rate_limit_status("ingest_ohlcv_futures_1m", "pre")
    ...
    logger.info("Minute OHLCV futures data ingestion completed.")
    record_rate_limit_status("ingest_ohlcv_futures_1m", "post")
