...
def get_last_ingested_datetime(
    ...
    query = f"""
        SELECT MAX(datetime)
        FROM market.cc_futures_open_interest_ohlc_1d
...
def ingest_daily_open_interest_data_for_instrument(
    ...
    """
    table_name = "market.cc_futures_open_interest_ohlc_1d"
    
    # API limits
    max_limit_per_call = 5000 # Max limit for daily open interest data
    ...
    if last_datetime_in_db:
        ...
        if first_open_interest_update_datetime:
            start_date_to_fetch = first_open_interest_update_datetime
            logger.info(
                f"No existing data for {mapped_instrument} on {market}. Backfilling from first open interest update datetime: {start_date_to_fetch.strftime('%Y-%m-%d %H:%M:%S UTC')}."
            )
        else:
            # Fallback to 2 years ago if first_open_interest_update_datetime is not available
            ...
            logger.info(
                f"No existing data for {mapped_instrument} on {market} and no first open interest update datetime. Backfilling from {start_date_to_fetch.strftime('%Y-%m-%d %H:%M:%S UTC')}."
            )

    # Determine the effective end timestamp for fetching
    # This is the minimum of yesterday's end and the instrument's last open interest update datetime
    effective_to_ts_date = yesterday_utc_date
    if last_open_interest_update_datetime and last_open_interest_update_datetime.date() < yesterday_utc_date:
        ...
        try:
            ...
            if data and data.get("Data"):
                ...
                if records:
                    ...
                    logger.info(
                        f"Successfully ingested {len(records)} daily open interest records for {mapped_instrument} on {market}."
                    )
                    ...
                    logger.info(
                        f"No new daily open interest data to ingest for {mapped_instrument} on {market} in this batch."
                    )
            else:
                logger.warning(f"No data received for {mapped_instrument} on {market} for this batch.")
        except Exception as e:
            logger.error(
                f"Error ingesting daily open interest data for {mapped_instrument} on {market}: {e}"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Ingest daily open interest futures data for specified or all exchanges and instruments."
    )
    parser.add_argument(
        "--exchanges",
        type=str,
    ...
    print(
        "Attempting to ingest daily open interest futures data..."
    )
    record_rate_limit_status("ingest_open_interest_futures_1d", "pre")
    ...
    logger.info("Daily open interest futures data ingestion completed.")
    record_rate_limit_status("ingest_open_interest_futures_1d", "post")
