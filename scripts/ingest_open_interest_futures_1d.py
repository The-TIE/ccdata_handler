...
def get_futures_instruments_from_db(
    ...
    try:
        ...
        if results:
            # Results are (exchange_internal_name, mapped_instrument_symbol, last_open_interest_update_datetime)
...
def main():
    ...
    if args.instruments:
        # When instruments are manually specified, we don't have last_open_interest_update_datetime from DB.
        # In this case, we assume they are active or we want to fetch up to yesterday.
        instruments_to_process = [
            (exchange, instrument.strip(), None) # last_open_interest_update_datetime is None
    ...
    for market, mapped_instrument, last_open_interest_update_datetime in instruments_to_process:
        ingest_daily_open_interest_data_for_instrument(
            futures_api_client, db_connection, market, mapped_instrument, last_open_interest_update_datetime
        )
