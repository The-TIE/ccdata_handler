...
def get_last_ingested_datetime(
    ...
    query = f"""
        SELECT MAX(datetime)
        FROM market.cc_futures_ohlcv_1h
...
def ingest_hourly_ohlcv_data_for_instrument(
    ...
    """
    table_name = "market.cc_futures_ohlcv_1h"
    
    # API limits
    max_limit_per_call = 2000 # Max limit for hourly OHLCV data (approx 83 days)
    ...
    if last_datetime_in_db:
        ...
        if first_trade_datetime:
            ...
            # Fallback to max_limit_per_call hours ago if first_trade_datetime is not available
    ...
    # This is the minimum of one hour ago and the instrument's last trade datetime
    effective_to_ts_datetime = one_hour_ago_utc
    if last_trade_datetime and last_trade_datetime < one_hour_ago_utc:
        ...
        try:
            ...
            if data and data.get("Data"):
                ...
                if records:
                    ...
                    logger.info(
                        f"Successfully ingested {len(records)} hourly OHLCV records for {mapped_instrument} on {market}."
                    )
                    ...
                    logger.info(
                        f"No new hourly OHLCV data to ingest for {mapped_instrument} on {market} in this batch."
                    )
            else:
                logger.warning(f"No data received for {mapped_instrument} on {market} for this batch.")
        except Exception as e:
            logger.error(
                f"Error ingesting hourly OHLCV data for {mapped_instrument} on {market}: {e}"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Ingest hourly OHLCV futures data for specified or all exchanges and instruments."
    )
    parser.add_argument(
        "--exchanges",
        type=str,
    ...
    print(
        "Attempting to ingest hourly OHLCV futures data..."
    )
    record_rate_limit_status("ingest_ohlcv_futures_1h", "pre")
    ...
    logger.info("Hourly OHLCV futures data ingestion completed.")
    record_rate_limit_status("ingest_ohlcv_futures_1h", "post")
