...
def create_target_table_if_not_exists():
    ...
    create_table_query = """
    CREATE TABLE IF NOT EXISTS market.cc_futures_funding_rate_ohlc_1m_new (
...
def load_parquet_file_to_target(parquet_file: str) -> int:
    ...
    INTO TABLE market.cc_futures_funding_rate_ohlc_1m_new
    ({column_mapping_str})
    FORMAT PARQUET;
    """
...
def verify_transfer_completion():
    ...
    with DbConnectionManager() as db:
        ...
        source_query = "SELECT COUNT(*) FROM market.cc_futures_funding_rate_ohlc_1m"
        ...
        target_query = "SELECT COUNT(*) FROM market.cc_futures_funding_rate_ohlc_1m_new"
...
def bulk_transfer_futures_funding_rate(
    output_dir: str = "/tmp/futures_funding_rate_parquet",
    chunk_days: int = 30,
    ...
    """
    logger.info(
        "Starting bulk transfer of futures Funding Rate data using Parquet format..."
    )
    ...
    try:
        ...
        while current_dt <= max_date:
            ...
            output_file = os.path.join(
                output_dir,
                f"futures_funding_rate_{current_dt.strftime('%Y%m%d')}_to_{chunk_end_dt.strftime('%Y%m%d')}.parquet",
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Bulk transfer futures Funding Rate data from SingleStore to Parquet and back."
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="/tmp/futures_funding_rate_parquet",
    ...
    if args.cleanup_only:
        ...
        bulk_transfer_futures_funding_rate(
