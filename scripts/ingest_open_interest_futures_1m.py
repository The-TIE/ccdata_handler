...
def get_futures_instruments_from_db(
    ...
    Returns a list of (exchange_internal_name, mapped_instrument_symbol, last_open_interest_update_datetime) tuples.
    """
    if not exchanges:
    ...
    try:
        query = db._load_sql("get_futures_instruments_open_interest.sql")
        ...
        logger.error("SQL script 'get_futures_instruments_open_interest.sql' not found.")
        return []
    except Exception as e:
...
def get_last_ingested_datetime(
    ...
    query = f"""
        SELECT MAX(datetime)
        FROM market.cc_futures_open_interest_ohlc_1m
...
def ingest_minute_open_interest_data_for_instrument(
    ...
    """
    Fetches minute open interest data for a specific futures instrument and ingests it into the database.
    Handles backfilling and live ingestion by paginating through available data.
    Adjusts to_ts based on last_open_interest_update_datetime for expired/retired contracts.
    """
    table_name = "market.cc_futures_open_interest_ohlc_1m"
    
    # API limits
    max_limit_per_call = 2000 # Max limit for minute open interest data (approx 33 hours)
    ...
    if last_datetime_in_db:
        start_datetime_to_fetch = last_datetime_in_db + timedelta(minutes=1)
        logger.info(
            f"Continuing ingestion for {mapped_instrument} on {market} from {start_datetime_to_fetch.strftime('%Y-%m-%d %H:%M:%S UTC')}."
        )
    else:
        # If no data exists, start backfill from the first open interest update datetime
        if first_open_interest_update_datetime:
            start_datetime_to_fetch = first_open_interest_update_datetime
            logger.info(
                f"No existing data for {mapped_instrument} on {market}. Backfilling from first open interest update datetime: {start_datetime_to_fetch.strftime('%Y-%m-%d %H:%M:%S UTC')}."
            )
        else:
            # Fallback to max_limit_per_call minutes ago if first_open_interest_update_datetime is not available
            two_thousand_minutes_ago = now_utc - timedelta(minutes=max_limit_per_call)
            start_datetime_to_fetch = two_thousand_minutes_ago.replace(second=0, microsecond=0)
            logger.info(
                f"No existing data for {mapped_instrument} on {market} and no first open interest update datetime. Backfilling from {start_datetime_to_fetch.strftime('%Y-%m-%d %H:%M:%S UTC')}."
            )

    # Determine the effective end timestamp for fetching
    # This is the minimum of the end of the previous minute and the instrument's last open interest update datetime
    # Determine the effective end timestamp for fetching
    ...
    if instrument_status == "ACTIVE":
        ...
        effective_to_ts_datetime = last_open_interest_update_datetime if last_open_interest_update_datetime else end_of_previous_minute

    current_to_ts = int(effective_to_ts_datetime.timestamp())
    
    # Loop to paginate and fetch all available data
    while True:
        # Calculate the number of minutes between start_datetime_to_fetch and effective_to_ts_datetime
        delta_minutes = int((effective_to_ts_datetime - start_datetime_to_fetch).total_seconds() / 60)
        ...
        limit = min(delta_minutes + 1, max_limit_per_call)
        
        # Adjust to_ts for the current batch to ensure we don't fetch beyond the effective_to_ts_datetime
        batch_to_ts = int((start_datetime_to_fetch + timedelta(minutes=limit - 1)).timestamp())
        ...
        try:
            ...
            if data and data.get("Data"):
                ...
                if records:
                    ...
                    logger.info(
                        f"Successfully ingested {len(records)} minute open interest records for {mapped_instrument} on {market}."
                    )
                    ...
                    logger.info(
                        f"No new minute open interest data to ingest for {mapped_instrument} on {market} in this batch."
                    )
            else:
                logger.warning(f"No data received for {mapped_instrument} on {market} for this batch.")
        except Exception as e:
            logger.error(
                f"Error ingesting minute open interest data for {mapped_instrument} on {market}: {e}"
            )
            break # Exit loop on error

        # Prepare for the next batch
        start_datetime_to_fetch = datetime.fromtimestamp(batch_to_ts, tz=timezone.utc) + timedelta(minutes=1)
...
def main():
    parser = argparse.ArgumentParser(
        description="Ingest minute open interest futures data for specified or all exchanges and instruments."
    )
    parser.add_argument(
        "--exchanges",
        type=str,
    ...
    print(
        "Attempting to ingest minute open interest futures data..."
    )
    record_rate_limit_status("ingest_open_interest_futures_1m", "pre")
    ...
    for market, mapped_instrument, last_open_interest_update_datetime, first_open_interest_update_datetime, instrument_status in instruments_to_process:
        ingest_minute_open_interest_data_for_instrument(
    ...
    latest_col = "collected_at" # Assuming 'collected_at' will be added to the open interest table for deduplication
    ...
    logger.info("Minute open interest futures data ingestion completed.")
    record_rate_limit_status("ingest_open_interest_futures_1m", "post")
