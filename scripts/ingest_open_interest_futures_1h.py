...
def get_last_ingested_datetime(
    ...
    query = f"""
        SELECT MAX(datetime)
        FROM market.cc_futures_open_interest_ohlc_1h
...
def ingest_hourly_open_interest_data_for_instrument(
    ...
    """
    table_name = "market.cc_futures_open_interest_ohlc_1h"
    
    # API limits
    max_limit_per_call = 2000 # Max limit for hourly open interest data (approx 83 days)
    ...
    if last_datetime_in_db:
        ...
        if first_open_interest_update_datetime:
            ...
            # Fallback to max_limit_per_call hours ago if first_open_interest_update_datetime is not available
        ...
        try:
            ...
            if data and data.get("Data"):
                ...
                if records:
                    ...
                    logger.info(
                        f"Successfully ingested {len(records)} hourly open interest records for {mapped_instrument} on {market}."
                    )
                    ...
                    logger.info(
                        f"No new hourly open interest data to ingest for {mapped_instrument} on {market} in this batch."
                    )
            else:
                logger.warning(f"No data received for {mapped_instrument} on {market} for this batch.")
        except Exception as e:
            logger.error(
                f"Error ingesting hourly open interest data for {mapped_instrument} on {market}: {e}"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Ingest hourly open interest futures data for specified or all exchanges and instruments."
    )
    parser.add_argument(
        "--exchanges",
        type=str,
    ...
    print(
        "Attempting to ingest hourly open interest futures data..."
    )
    record_rate_limit_status("ingest_open_interest_futures_1h", "pre")
    ...
    logger.info("Hourly open interest futures data ingestion completed.")
    record_rate_limit_status("ingest_open_interest_futures_1h", "post")
