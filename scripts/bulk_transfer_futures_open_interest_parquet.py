...
def create_target_table_if_not_exists():
    ...
    create_table_query = """
    CREATE TABLE IF NOT EXISTS market.cc_futures_open_interest_ohlc_1m_new (
        ...
        open_oi_contracts DOUBLE,
        high_oi_contracts DOUBLE,
        low_oi_contracts DOUBLE,
        close_oi_contracts DOUBLE,
        open_oi_quote DOUBLE,
        high_oi_quote DOUBLE,
        low_oi_quote DOUBLE,
        close_oi_quote DOUBLE,
        open_mark_price DOUBLE,
        high_oi_mark_price DOUBLE,
        high_mark_price DOUBLE,
        high_mark_price_oi DOUBLE,
        high_quote_mark_price DOUBLE,
        low_oi_mark_price DOUBLE,
        low_mark_price DOUBLE,
        low_mark_price_oi DOUBLE,
...
def export_chunk_to_parquet(
    ...
    SELECT
        ...
        high_mark_price,
        high_mark_price_oi,
        high_quote_mark_price,
        low_oi_mark_price,
        low_mark_price,
        low_mark_price_oi,
        low_quote_mark_price,
        close_mark_price,
...
def load_parquet_file_to_target(parquet_file: str) -> int:
    ...
    INTO TABLE market.cc_futures_open_interest_ohlc_1m_new
    ({column_mapping_str})
    FORMAT PARQUET;
    """
...
def verify_transfer_completion():
    ...
    with DbConnectionManager() as db:
        ...
        source_query = "SELECT COUNT(*) FROM market.cc_futures_open_interest_ohlc_1m"
        ...
        target_query = (
            "SELECT COUNT(*) FROM market.cc_futures_open_interest_ohlc_1m_new"
        )
...
def bulk_transfer_futures_open_interest(
    output_dir: str = "/tmp/futures_open_interest_parquet",
    chunk_days: int = 30,
    ...
    """
    logger.info(
        "Starting bulk transfer of futures Open Interest data using Parquet format..."
    )
    ...
    try:
        ...
        while current_dt <= max_date:
            ...
            output_file = os.path.join(
                output_dir,
                f"futures_open_interest_{current_dt.strftime('%Y%m%d')}_to_{chunk_end_dt.strftime('%Y%m%d')}.parquet",
            )
...
def cleanup_existing_parquet_files(output_dir: str):
    ...
    logger.info(f"Cleaning up existing Parquet files in {output_dir}...")
...
def main():
    parser = argparse.ArgumentParser(
        description="Bulk transfer futures Open Interest data from SingleStore to Parquet and back."
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="/tmp/futures_open_interest_parquet",
        help="Directory to store temporary Parquet files.",
    )
    parser.add_argument(
        "--chunk_days",
        type=int,
        default=30,
        help="Number of days per chunk for processing.",
    )
    parser.add_argument(
        "--no_cleanup",
        action="store_false",
        dest="cleanup_files",
        help="Do not delete Parquet files after loading.",
    )
    parser.add_argument(
        "--start_date",
        type=str,
        help="Optional start date (YYYY-MM-DD) for data transfer.",
    )
    parser.add_argument(
        "--end_date",
        type=str,
        help="Optional end date (YYYY-MM-DD) for data transfer.",
    )
    parser.add_argument(
        "--cleanup_only",
        action="store_true",
        help="Only clean up existing Parquet files and exit.",
    )
    ...
    if args.cleanup_only:
        ...
        bulk_transfer_futures_open_interest(
